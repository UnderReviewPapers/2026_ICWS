...
export const useMCPBedrock = (bedrockApiEndpoint: string): MCPBedrockHook => {
  ...
  const isCUDOperation = (toolCall: ToolCall): boolean => {
    const cudKeywords = ['update', 'delete', 'create', 'edit', 'remove', 'write'];
    const toolDef = allTools?.find(t => t.name === toolCall.name);
    if (toolDef?.semantic && toolDef.semantic.effect) {
      console.log("Checking if tool call is CUD operation:", toolDef);
      return cudKeywords.some(keyword => toolDef.semantic?.effect?.toLowerCase().includes(keyword));
    }
    return cudKeywords.some(keyword => toolCall.name.toLowerCase().includes(keyword));
  ...
  const compactResult = async (toolRes: string, tool_calls: any, call: ToolCall): Promise<string> => {

    const sessionId = mcpClient.sessionId!;

    const toolDef = allTools?.find(t => t.name === call.name);
    let producesInfo = null;
    if (toolDef?.orchestration && toolDef.orchestration?.produces) {
      console.log("Compacting ", toolDef);
      producesInfo = toolDef.orchestration.produces;
    }

    const prompt = buildSystemPromptCompactToolResult(tool_calls, userQueryRef.current, producesInfo);
  ...
  const handlePermissionConfirm = async () => {
    ...
    currentToolCompactResultsRef.current.set(call.id, await compactResult(result.content, currentToolPlanRef.current, call));
