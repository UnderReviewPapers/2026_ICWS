...
Each tool also provides:
- intent: the functional role of the operation (e.g., resolve, read, update)
- entities: the domain objects involved
- preconditions: constraints that must be satisfied before execution
- postconditions: state guaranteed after execution
- produces: symbolic outputs created by the operation (e.g., storeId, storeDetails)
- consumes: symbolic inputs required by the operation
- dependsOn: tools that must be executed first
- destructiveUpdate: boolean indicating whether the update will overwrite or delete existing data.

You MUST use this metadata to choose tools, order them and resolve dependencies.
...
- Do NOT invoke a tool if its preconditions are not satisfied.
- Use dependsOn, produces and consumes to automatically build a valid execution workflow.
- Respect dependsOn when ordering tool_calls.
- Use produces and consumes to link tool calls together.

Tool calls MUST be ordered according to:
1. dependsOn
2. produced/consumed values
3. preconditions
If the order is invalid, the plan is incorrect.
...
export function buildSystemPromptCompactToolResult(toolPlan:any, userQuery: string, producesInfo: string[] | null): string {
  return `
...
${producesInfo && false
  ? `### The current TOOL produces the following conceptual entities and fields, that need to be retained but without change names of fields:
${JSON.stringify(producesInfo, null, 2)}`
  : ''}
