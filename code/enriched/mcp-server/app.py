...
skip_to = None
# skip_to = ["StoresController_updateStore", "CustomersController_getCustomer", "StoresController_getGroups"]#, "DevicesController_CreateDevice"]
...
def generate_tool_function(api_url, operation_id, path, method, parameters, request_body, requires_auth, details, swagger, tags, summary=None, input_example=None, output_example=None):
    ...
    x_semantic = details.get("x-semantic", {})
    if x_semantic:
        x_semantic.pop("functionalDescription", None)
        descr_payload["semantic"] = x_semantic
    
    x_orchestration = details.get("x-orchestration", {})
    if x_orchestration:
        x_orchestration.pop("requiresConfirmation", None)
        descr_payload["orchestration"] = x_orchestration
